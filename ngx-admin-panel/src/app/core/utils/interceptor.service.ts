...
export class Interceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (req.url.indexOf('assets') > -1 || req.url.indexOf('.json') > -1) {
      ...
      if (this.cookieSer.check(CookieEnum.AdminToken)) {
        this.newToken = this.cookieSer.get(CookieEnum.AdminToken);
      } else if (this.cookieSer.check(CookieEnum.AdminRefresh)) {
        ...
        tap({
          error: err => {
            if (err instanceof HttpErrorResponse) {
              if (err.status === 401) {
                this.translate.get('Account.Logged').subscribe(data => {
                  const message = data;
                  this.global.notificationMessage(ResponseEnum.Failed, null, message);
                });
                this.accountSer.logout();
              }
            }
